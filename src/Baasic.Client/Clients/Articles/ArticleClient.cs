...
namespace Baasic.Client.ArticleModule
{
    ...
    public class ArticleClient : ClientBase, IArticleClient
    {
        ...
        public virtual Task<ArticleTagEntry> AddTagToArticleAsync(Guid articleId, string tag)
        {
            using (IBaasicClient client = BaasicClientFactory.Create(Configuration))
            {
                return client.PostAsync<ArticleTagEntry>(client.GetApiUrl(String.Format("{0}/{{0}}/tag/{{1}}", ModuleRelativePath), articleId, tag), null);
            }
        }
        ...
        public virtual Task<ArticleTagEntry> AddTagToArticleAsync(ArticleTagEntry entry)
        {
            using (IBaasicClient client = BaasicClientFactory.Create(Configuration))
            {
                return client.PostAsync<ArticleTagEntry>(client.GetApiUrl(String.Format("{0}/tag/", ModuleRelativePath)), entry);
            }
        }

        /// <summary>
        /// Asynchronously get <see cref="ArticleTagEntry"/> entries.
        /// </summary>
        ...
        public virtual Task<CollectionModelBase<ArticleTagEntry>> GetTagEntriesAsync(string searchQuery = "",
            int page = DefaultPage, int rpp = MaxNumberOfResults,
            string sort = DefaultSorting, string embed = DefaultEmbed)
        {
            using (IBaasicClient client = BaasicClientFactory.Create(Configuration))
        ...
        public virtual Task<ArticleTagEntry> GetTagEntryAsync(Guid articleId, object key, string embed = "")
        {
            using (IBaasicClient client = BaasicClientFactory.Create(Configuration))
            {
                return client.GetAsync<ArticleTagEntry>(client.GetApiUrl(String.Format("{0}/{{0}}/tag/{{1}}", ModuleRelativePath), articleId, key));
            }
        }
